...
export const model = {
  ...
  upgrades: [
    ...
    },
    {
      ...
      description:
        "Removed: breakglassJustification, policy, useDefault, percent, revision, type, manualInstanceCount, containers, args, baseImageUri, buildInfo, functionTarget, sourceLocation, command, dependsOn, env, value, valueSource, image, livenessProbe, failureThreshold, grpc, port, service, httpGet, httpHeaders, path, port, initialDelaySeconds, periodSeconds, tcpSocket, port, timeoutSeconds, ports, containerPort, readinessProbe, failureThreshold, grpc, port, service, httpGet, httpHeaders, path, port, initialDelaySeconds, periodSeconds, tcpSocket, port, timeoutSeconds, resources, cpuIdle, limits, startupCpuBoost, sandboxLauncher, sourceCode, cloudStorageSource, bucket, generation, object, inlinedSource, sources, startupProbe, failureThreshold, grpc, port, service, httpGet, httpHeaders, path, port, initialDelaySeconds, periodSeconds, tcpSocket, port, timeoutSeconds, volumeMounts, mountPath, subPath, workingDir, encryptionKey, encryptionKeyRevocationAction, encryptionKeyShutdownDuration, gpuZonalRedundancyDisabled, nodeSelector, accelerator, revision, serviceAccount, serviceMesh, mesh, volumes, cloudSqlInstance, instances, emptyDir, medium, sizeLimit, gcs, bucket, mountOptions, readOnly, nfs, path, readOnly, server, secret, defaultMode, items, secret, vpcAccess, connector, egress, networkInterfaces, network, subnetwork, tags",
      upgradeAttributes: (old: Record<string, unknown>) => {
        const {
