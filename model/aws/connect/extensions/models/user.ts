...
export const UserProficiencySchema = z.object({
...
export const AutoAcceptConfigSchema = z.object({
...
export const AfterContactWorkConfigSchema = z.object({
  AfterContactWorkTimeLimit: z.number().int().min(0).describe(
    "The After Call Work (ACW) timeout setting, in seconds.",
  ).optional(),
});

export const AfterContactWorkConfigPerChannelSchema = z.object({
  Channel: z.enum(["VOICE", "CHAT", "TASK", "EMAIL"]).describe(
    "The channels that agents can handle in the Contact Control Panel (CCP).",
  ),
...
export const PersistentConnectionConfigSchema = z.object({
  Channel: z.enum(["VOICE", "CHAT", "TASK", "EMAIL"]).describe(
    "The channels that agents can handle in the Contact Control Panel (CCP).",
  ),
...
export const VoiceEnhancementConfigSchema = z.object({
  Channel: z.enum(["VOICE", "CHAT", "TASK", "EMAIL"]).describe(
    "The channels that agents can handle in the Contact Control Panel (CCP).",
  ),
